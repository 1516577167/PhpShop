...
class Cart extends Model
{
	public function addToCart($goodsId,$goodsAttr='',$goodsNum=1){
      ...
      if(isset($cart[$key])){
         $cart[$key]+=$goodsNum;//如果在此之前已经为该商品加入了购物车那么在次添加只需要修改商品数量
      }else{
         $cart[$key]=$goodsNum;//第一次添加直接让它等于添加的数量
      }
      // if($cart[$key]){
      ...
      setcookie('cart',serialize($cart),$aMonth,'/');  //cookie不能存储数组，serialize将数字组序列化
   }
      ...
      if($doGoods){
         ...
         foreach ($cart as $k => $v) {
            if(!in_array($k,$doGoodsArr)){
               ...
               foreach ($cart as $k => $v) {
               $arr=explode('-',$k);//$arr[0]是商品ID，如果存在第二个元素的话$arr[1]代表商品单选属性ID字符串
   ...
   public function getGoodsListInCart($doGoods=''){
      ...
      foreach ($cart as $k => $v) {
         ...
         $_cart[$k]['markte_price']=$goodsInfo['markte_price'];
         $_cart[$k]['goods_num']=$v;
         $_cart[$k]['goods_id']=$goodsInfo['id'];
         $_cart[$k]['goods_id_attr_id']=$k;  //用于处理复选框问题
         ...
         if($arr[1]){
            ...
            $goodsAttrRes=db('goods_attr')->alias('ga')->field('ga.*,a.attr_name')->join('attr a',"ga.attr_id=a.id")->where('ga.id','in',$arr[1])->select();
            foreach ($goodsAttrRes as $k1 => $v1) {
               $goodsStr[]=$v1['attr_name'].':'.$v1['attr_value'].'(￥'.$v1['attr_price'].'元)';
               $goodsAttrPrice+=$v1['attr_price'];
            }
            $goodsStr=implode('<br />',$goodsStr);
            $_cart[$k]['goods_attr_str']=$goodsStr;
            $_cart[$k]['member_price']+=$goodsAttrPrice;
